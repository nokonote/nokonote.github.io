...
- 切り口: マンネリ防止のため、以下の目標配分に沿うようカテゴリを選択し `category` フロントマターに明示する。執筆前に `src/content/blog/` 内の既存記事の `category` 内訳を集計し、目標配分より実際の比率が低いカテゴリを優先的に選ぶ（完全な機械的割り当てではなく、多少のランダム性を加えてよいが、目標配分から大きく乖離させない）
  1. モデルコース提案・1泊2日/2泊3日など (`itinerary`) — 目標30%
  2. 観光スポット紹介 (`spot`) — 目標20%
  3. 温泉特集 (`onsen`) — 目標20% ※露天風呂・日帰り温泉・秘湯・温泉街散策など、温泉そのものにフォーカスした記事
  4. グルメ・ご当地グルメ特集 (`gourmet`) — 目標10%
  5. 一人旅/友達旅ならではのTips (`tips`) — 目標10%
  6. 費用・予算シミュレーション (`budget`) — 目標8%
  7. 持ち物・準備リスト (`packing`) — 目標2%
  8. 体験談風エッセイ (`essay`) ※目標配分の対象外。ユーザーが明示的に指定した場合のみ使用する（自動選択・ランダム選択の対象にしない）
...
- 本ブログの記事は筆者の実体験の旅行記ではなく、モデルプラン・おすすめプランの「提案」であることを踏まえた文体にする
  - 「〜に行ってきました」「〜を食べました」のような一人称の体験談的表現は避け、「〜がおすすめです」「〜というプランはいかがでしょうか」「〜を巡るコースを紹介します」のような提案型の表現に統一する
  - タイトル・見出しも「〜旅行記」ではなく「〜モデルプラン」「〜おすすめコース」といった表現にする
  - 感想表現（「感動しました」「美味しかったです」など一人称の感想）は、「〜と評判です」「〜が楽しめます」のような客観的な紹介表現に置き換える
  - 読者が「筆者の実体験談」と誤解しないよう、リード文（冒頭の導入段落）に「〜のプランをご紹介します」「〜を提案します」など、記事が“提案”であることが伝わる一文を必ず入れる
- 記事の長さは切り口によって変える
...
- 執筆10回のうち1〜2回程度の頻度で、長文記事（3000〜5000文字程度。複数スポットや情報を深掘りし、じっくり読ませる特集記事にする）を作成する。長文回にするかどうかは執筆前にランダムまたは任意で判断してよく、通常回との緩急をつける
  - 長文回は必ず `itinerary`（モデルコース提案）カテゴリにする。複数日程・複数スポットを深掘りできるitineraryとの相性がよいため
  - カテゴリの目標配分（上記「切り口」参照）を集計する際は、長文回もitinerary記事1本としてカウントする
...
- イベントの開催期間・営業時間、料金、アクセスの所要時間、歴史的事実（創業年など）といった検証可能な具体的事実は、一般知識からの推測で書かず、執筆時にWebSearch等で裏取りする
- 裏付けが取れない場合は、断定的な数値を書かず「〜円程度」「〜時間ほど」といった相場感の表現にとどめる
- 修正を指摘された場合は、該当記事だけでなく他の既存記事にも同種の誤りがないか横断的に確認する
- 記事一覧は `pubDate` の降順で新しい記事が上に表示される。同日に複数記事を作成する場合は、公開順が意図通りになるよう `pubDate` に時刻を含める（例: `2026-08-07T09:00:00+09:00`）
...
- 写真選定は次の優先順位で行う。1. 記事の舞台となる場所に実際に関係のある写真（その土地で撮影されたもの、または同一エリア内のもの）を最優先とし、無関係な地域の汎用的な「イメージ写真」をその場所の写真であるかのように使わない。2. 人物より風景・建築・食べ物など被写体そのものが主役の写真を優先する。特に入浴シーンに人物が写っている写真は、温泉特集（`onsen`）の記事であっても避ける
- 場所に厳密に合致する写真がPexels等で見つからない場合は、無理に別カットを追加せず、heroImageのみ（または本文中で使い回す）で済ませてよい
