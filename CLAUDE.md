...
- 切り口: マンネリ防止のため、以下をローテーションまたはランダムで選択し `category` フロントマターに明示する
  ...
  7. 温泉特集 (`onsen`) ※露天風呂・日帰り温泉・秘湯・温泉街散策など、温泉そのものにフォーカスした記事
  8. 体験談風エッセイ (`essay`) ※このカテゴリはローテーション対象外。ユーザーが明示的に指定した場合のみ使用する（自動選択・ランダム選択の対象にしない）
...
- 記事の長さは切り口によって変える
  ...
  - 観光スポット紹介、グルメ特集、費用シミュレーション、温泉特集（`spot`, `gourmet`, `budget`, `onsen`）: 1200〜2000文字程度
- タイトル・meta descriptionはSEOを意識して簡潔にする
- 執筆10回に1回程度の頻度で、カテゴリを問わず長文記事（3000〜5000文字程度。複数スポットや情報を深掘りし、じっくり読ませる特集記事にする）を作成する。長文回にするかどうかは執筆前にランダムまたは任意で判断してよく、通常回との緩急をつける
...
- Pexels等からダウンロードした画像は、そのままの解像度（数千px・数MB）でコミットしない
- 幅1600pxを上限にリサイズし、JPEG品質を落としてから `public/images/blog/<slug>/` に保存する（例: `ffmpeg -i input.jpg -vf "scale='min(1600,iw)':-2" -q:v 4 output.jpg`）
- 1枚あたりの目安は数百KB程度に収める
